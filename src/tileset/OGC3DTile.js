...
import { resolveImplicite } from './implicit/ImplicitTileResolver.js'; // ok
import { SplatsMesh } from '../splats/SplatsMesh'; // ok
var averageTime = 0;
...
const transformedProjection = new Matrix4();
const frustum = new Frustum();
const transformedCameraPosition = new Vector3();
 ...
 * @extends {Object3D}
 */
class OGC3DTile extends Object3D {
  /**
   ...
   * @param {Camera} [properties.cameraOnLoad = undefined] - optional the camera used when loading this particular sub-tile
   ...
   * @param {Renderer} [properties.renderer = undefined] - optional the renderer used to display the tileset. Used to infer render resolution at runtime and to instantiate a ktx2loader on the fly if not provided.
  ...
  constructor(properties) {
    ...
    if (!!properties.domWidth && !!properties.domHeight) {
      this.rendererSize = new Vector2(properties.domWidth, properties.domHeight);
    } else {
      this.rendererSize = new Vector2(1000, 1000);
    }
    ...
    if (this.occlusionCullingService) {
      ...
      this.colorID =
        (MathUtils.clamp(self.color.r * 255, 0, 255) << 16) ^
        (MathUtils.clamp(self.color.g * 255, 0, 255) << 8) ^
        (MathUtils.clamp(self.color.b * 255, 0, 255) << 0);
    }
    if (this.static) {
  ...
  async _setup(properties) {
    ...
    if (!!self.json.boundingVolume) {
      if (!!self.json.boundingVolume.box) {
        ...
        self.boundingVolume = new Sphere(
          new Vector3(tempVec1.x, tempVec1.y, tempVec1.z),
    ...
    if (self.level > 0 && self.drawBoundingVolume) {
      ...
      if (this.boundingVolume.aabb) {
        ...
        self.bbox = new Box3Helper(box, new Color(Math.random(), Math.random(), Math.random()));
   ...
   * @param {Camera} camera a camera that the tileset will be rendered with.
    ...
    if (this.refine == 'REPLACE') {
      switch (this.loadingStrategy) {
        case 'IMMEDIATE':
          ...
          this._statsImmediate(maxLOD, numTiles, percentageLoaded, numTilesRendered);
          break;
        default:
  ...
  _loadMeshImmediate() {
    ...
    if (self.shouldBeVisible) {
      if (self.meshContent.length < self.hasMeshContent && self.contentURL.length == 0) {
        self.deleted = false;
  ...
  _update(camera, frustum) {
    ...
    if (!self.isSetup) return;
    // let dist = self.boundingVolume.distanceToPoint(new Vector3(3980, 4980.416656099139, 3.2851604304346775));
    // if (dist< 1) {
    //     self._changeContentVisibility(false);
    ...
    function _updateTree(metric) {
      ...
      if (
        !self.hasMeshContent ||
        (metric <= self.geometricErrorMultiplier * self.geometricError && self.meshContent.length > 0)
      ) {
        if (!!self.json && !!self.json.children && self.childrenTiles.length != self.json.children.length) {
          self._loadJsonChildren(camera);
          return;
  ...
  _isReady() {
    ...
    if (this.meshContent.length < this.hasMeshContent) {
      console.log('mesh content not yet loaded');
      return false;
    }
