...
async function resolveImplicite(rootTile, url) {
  ...
  let subtreeUri;
  if (isQuad) {
    subtreeUri = subtreeUriTemplate.replace('{level}', 0).replace('{x}', 0).replace('{y}', 0);
  }
  if (!isQuad) {
    subtreeUri = subtreeUriTemplate.replace('{level}', 0).replace('{x}', 0).replace('{y}', 0).replace('{z}', 0);
  }
  ...
  async function buildChildren(parent) {
    ...
    if ((parent.localAddress.level + 1) % rootTile.root.implicitTiling.subtreeLevels == 0) {
      // end of subtree
...
function splitBox(isQuad, box, address) {
  ...
  const origin = [
    center[0] - vectors[0][0] - vectors[1][0] - vectors[2][0],
    center[1] - vectors[0][1] - vectors[1][1] - vectors[2][1],
    center[2] - vectors[0][2] - vectors[1][2] - vectors[2][2],
  ];
